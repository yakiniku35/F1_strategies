...
- 預測各大獎賽的排位結果
- 分析賽道進站策略（提前進站、留守策略等）
- 瀏覽任一年度的 F1 賽程表（透過 FastF1 取得，離線時使用內建賽程）
- 查看完整的完賽成績表與世界錦標賽積分
- 產生名次變化圖與輪胎策略圖
- 透過 AI 助理詢問 F1 相關知識
...
### 🏁 完賽成績與積分
比賽結束時顯示最終排名、與冠軍的差距、退賽原因與世界錦標賽積分
（含最快圈加分），可匯出成 JSON 或 CSV。

### 📊 比賽圖表
- **名次變化圖：** 逐圈名次折線圖，以車隊顏色區分，同隊第二車以虛線表示
- **輪胎策略圖：** 每位車手的進站區間與輪胎配方一覽

### ⏱️ 時間軸
回放視窗底部的進度條可直接點擊或拖曳跳到比賽的任何時刻，
並以顏色標示黃旗、安全車與紅旗區間。

### 📅 賽程表
檢視任一年度的 F1 完整賽程與賽事資訊。賽程與車手陣容會透過 FastF1
取得；若無網路或該年度賽程尚未公布，則自動改用內建賽程並明確提示。
...
測試涵蓋純邏輯部分（成績與積分、輪胎退化、策略評分、賽程載入、
退賽模擬、回放內插），完全不需要網路。若電腦上沒有安裝 FastF1、
scikit-learn 等較重的套件，`tests/conftest.py` 會自動以替身模組代替，
所以在精簡環境也能執行。每次 push 時 CI 也會跑同一套測試。
